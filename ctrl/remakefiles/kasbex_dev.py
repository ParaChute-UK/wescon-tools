...
def regrid_inputs(case, radar, batch_idx):
    ...
    return {
        'radar_paths': paths,
        'radarnet': conf.PATHS['datadir'] / 'remake3'
...
def regrid_camra_kepler_l1(inputs, outputs, case, radar, batch_idx):
    ...
    for i, radar_path in enumerate(radar_paths):
        ...
        to_netcdf_tmp_then_copy(ds, Path(outputs[f'gridded_data{i}']))


def find_matches_inputs(case):
    inputs = {}
    for radar in ['camra', 'kepler']:
        for batch_idx in range(len(cpmap(case, radar))):
            inputs.update(regrid_outputs(case, radar, batch_idx))
    return inputs


@rule(
    inputs=find_matches_inputs,
    outputs={'scans': str(conf.PATHS['kasbexoutdir'] / 'camra_kepler_scans/camra_kepler_scans_{case}.hdf')},
    matrix={'case': conf.KASBEX_CASES},
    depends_on=[regrid_camra_kepler_l1],
    uses={'get_time_az': get_time_az, 'find_matches': find_matches},
)
def match_camra_kepler_scans(inputs, outputs, case):
...
@rule(
    inputs=match_camra_kepler_scans.outputs,
    outputs={'dummy': str(conf.PATHS['kasbexoutdir'] / 'nonmatch_analysis/{case}/nonmatch_analysis_dummy_{case}.txt')},
    matrix={'case': conf.KASBEX_CASES},
    depends_on=[match_camra_kepler_scans],
    uses={'plot_rhi': plot_rhi},
)
def analyse_nonmatches(inputs, outputs, case):
...
@rule(
    inputs=match_camra_kepler_scans.outputs,
    outputs={'dummy': str(conf.PATHS['kasbexoutdir'] / 'match_analysis/{case}/match_analysis_dummy_{case}.txt')},
    matrix={'case': conf.KASBEX_CASES},
    depends_on=[match_camra_kepler_scans],
    uses={'plot_joint': plot_joint, 'plot_overlaid': plot_overlaid},
)
def analyse_matches(inputs, outputs, case):
    scans = pd.read_hdf(inputs['scans'], 'scans')
    matches = scans[scans.match]
    outdir = Path(outputs['dummy']).parent
...
@rule(
    inputs=match_camra_kepler_scans.outputs,
    outputs={'dummy': str(conf.PATHS['kasbexoutdir'] /
                          'matches_for_case_analysis/{case}/match_analysis_dummy_{case}.txt')},
    matrix={'case': conf.KASBEX_CASES},
    depends_on=[match_camra_kepler_scans],
    uses={'plot_joint': plot_joint},
)
def analyse_matches_for_case(inputs, outputs, case):
    scans = pd.read_hdf(inputs['scans'], 'scans')
    matches = scans[scans.match]
    if len(matches):
        outdir = Path(outputs['dummy']).parent
...
@rule(
    inputs={f'scan_{c}': str(match_camra_kepler_scans.outputs['scans']).format(case=c) for c in conf.KASBEX_CASES},
    outputs={'dummy': str(conf.PATHS['kasbexoutdir'] / 'match_comaprison/match_comparison_dummy.txt')},
    depends_on=[match_camra_kepler_scans],
)
def compare_matches(inputs, outputs):
    outdir = Path(outputs['dummy']).parent
